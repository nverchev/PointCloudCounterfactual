...
class TransformerEncoder(BasePointEncoder):
    ...
    def __init__(self) -> None:
        ...
        self.final_conv = PointsConvLayer(
            sum(self.conv_dims), self.code_dim * self.n_codes, act_cls=self.act_cls, norm_cls=self.norm_cls
        )
        self.proj_codes = LinearLayer(self.code_dim, self.proj_dim)
        self.proj_input = LinearLayer(IN_CHAN, self.proj_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        queries = self.proj_codes(x_max.view(batch, self.n_codes, self.code_dim))
